...
//	cout << "@@@@@@@@@@@@@@@@" << endl;
	...
	for (it_t i1 = words1.begin(); i1 != words1.end(); ++i1) {
		...
		chunk.words[0] = w1;
		m_pos += w1->nbytes;
//		cout << "1 m_pos: " << m_pos  << "\t" << w1->nbytes << "\t" << w1->text << endl;
		if (m_pos < m_text_length) {
			...
			for (it_t i2 = words2.begin(); i2 != words2.end(); ++i2) {
				...
				chunk.words[1] = w2;
				m_pos += w2->nbytes;
//				cout << "2 m_pos: " << m_pos  << "\t" << w2->nbytes << "\t" << w2->text << endl;
				if (m_pos < m_text_length) {
					vec_t words3 = find_match_words();
					for (it_t i3 = words3.begin(); i3 != words3.end(); ++i3) {
						w3 = *i3;
//						cout << "3 m_pos: " << m_pos  << "\t" << w3->nbytes << "\t" << w3->text << endl;
...
//	cout << "chunks size: " << chunks.size() << "\t" << chunks[0].n << endl;
...
int Algorithm::next_char() {
	...
	int size = 1;
	if (ch >= 0) {
		while (m_pos + size < m_text_length && m_text[m_pos + size] >= 0) {
			size++;
		}
		return size;
	} else {
